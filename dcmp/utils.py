...
def create_task_instance_by_dag_code(dag_code, dag_name, task_name, execution_date=None):
    with TemporaryDirectory(prefix='dcmp_dag_') as tmp_dir:
        with NamedTemporaryFile(dir=tmp_dir) as f:
            ...
            f.flush()
            ti = create_task_instance(dag_name, task_name, execution_date=execution_date,
                                      dag_folder=os.path.join(tmp_dir, f.name), include_examples=False)
    return ti
...
def search_conf_iter(search, conf, key=None, task_name=""):
    if isinstance(conf, str):
        ...
        for item in conf:
            ...
            for result_task_name, result_key, result_line in search_conf_iter(search, item, key=key,
                                                                              task_name=task_name):
                yield result_task_name, result_key, result_line
    elif isinstance(conf, dict):
        for item_key in sorted(conf.keys()):
            item_value = conf[item_key]
            for result_task_name, result_key, result_line in search_conf_iter(search, item_value, key=item_key,
                                                                              task_name=task_name):
                yield result_task_name, result_key, result_line
