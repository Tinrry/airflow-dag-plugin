...
class DcmpUserProfile(Base):
    __tablename__ = "dcmp__userprofile"
...
def sync_profiles(action=None, target=None):
    ...
    for user_id in no_profile_user_ids:
        ...
        session.add(profile)
    session.query(DcmpUserProfile).filter(DcmpUserProfile.user_id.in_(no_user_user_ids)).delete(
...
if __package__:
    try:
        ...
        logging.warning("Run python {AIRFLOW_HOME}/plugins/dcmp/tools/upgradedb.py first")
    ...
    if configuration.conf.get('api', 'auth_backend').endswith('dcmp.auth.backends.password_auth'):
        from dcmp.auth.backends.password_auth import PasswordUser

        listen(PasswordUser, 'after_insert',
               lambda mapper, connection, target: sync_profiles(action="insert", target=target))
        listen(PasswordUser, 'after_delete',
               lambda mapper, connection, target: sync_profiles(action="delete", target=target))
