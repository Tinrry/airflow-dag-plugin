...
try:
    DAG_CREATION_MANAGER_LINE_INTERPOLATE = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_LINE_INTERPOLATE')
except Exception as e:
    DAG_CREATION_MANAGER_LINE_INTERPOLATE = "basis"

HOSTNAME = socket.gethostname()
AIRFLOW_DAGS_FOLDER = configuration.conf.get('core', 'DAGS_FOLDER')
DAG_CREATION_MANAGER_DEPLOYED_DAGS_FOLDER = os.path.join(AIRFLOW_DAGS_FOLDER, "deployedDags")
DAG_CREATION_MANAGER_QUEUE_POOL_STR = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_QUEUE_POOL')
...
DAG_CREATION_MANAGER_CATEGORY_STR = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_CATEGORY')
...
DAG_CREATION_MANAGER_TASK_CATEGORY_STR = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_TASK_CATEGORY')
...
DAG_CREATION_MANAGER_QUEUE_POOL_MR_QUEUE_STR = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_QUEUE_POOL_MR_QUEUE')
...
DAG_CREATION_MANAGER_DEFAULT_EMAIL_STR = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_DEFAULT_EMAIL')
DAG_CREATION_MANAGER_DEFAULT_EMAILS = [email.strip() for email in DAG_CREATION_MANAGER_DEFAULT_EMAIL_STR.split(",") if email.strip()]

try:
    DAG_CREATION_MANAGER_NEED_APPROVER = configuration.conf.getboolean('dag_creation_manager', 'DAG_CREATION_MANAGER_NEED_APPROVER')
except Exception as e:
    DAG_CREATION_MANAGER_NEED_APPROVER = False

try:
    DAG_CREATION_MANAGER_CAN_APPROVE_SELF = configuration.conf.getboolean('dag_creation_manager', 'DAG_CREATION_MANAGER_CAN_APPROVE_SELF')
except Exception as e:
    DAG_CREATION_MANAGER_CAN_APPROVE_SELF = True

try:
    DAG_CREATION_MANAGER_DAG_TEMPLATES_DIR = configuration.conf.get('dag_creation_manager', 'DAG_CREATION_MANAGER_DAG_TEMPLATES_DIR')
except Exception as e:
